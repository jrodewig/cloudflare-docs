...
You must create a secret key to connect to the database from your development environment.
...
Before developing your Worker, add your Fauna database key as a [secret](/workers/configuration/secrets/).
...
- Select the _integrations_ tab and click on the _Fauna_ integration.
...
- Select the Fauna database you created earlier.
- Select `server` role as your database role.
- Select _Finish_.
- Navigate to settings tab and select _Variables_. Notice that a new variable `DATABASE_KEY`
  is added to your Worker.

The Fauna `DATABASE_KEY` secret is now injected automatically into your Worker code at runtime.

:::note

The generated `DATABASE_KEY` secret can also be managed from the Fauna dashboard.
To Access it login to your [Fauna dashboard](https://dashboard.fauna.com/) and navigate to
your database and then select _Keys_.
...
You can access the `DATABASE_KEY` environment variable from `c.env.DATABASE_KEY`. Workers run on a [custom JavaScript runtime](https://developers.cloudflare.com/workers/runtime-apis/nodejs/) instead of Node.js, so you cannot use `process.env` to access your environment variables.
